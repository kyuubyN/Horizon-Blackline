...
(defn preflight!
  ...
  (let [intent-hash (bdr/sha256 (canonical/encode intent))]
    (cond
      ...
      (not (constant-time-eq? intent-hash (:input-hash authorization))) (throw (ex-info "Intent differs from authorization" {:reason-code :INTENT_HASH_MISMATCH}))
...
(defn order-arguments
  ...
  (let [{:keys [symbol quantity entry-price stop-price]} intent
        ...
        with-limit (cond-> base
                     (#{:limit :stop-limit} order-type) (assoc :limit_price entry-price)
                     (#{:stop :stop-limit} order-type) (assoc :stop_price stop-price))]
    (case mcp-tool
